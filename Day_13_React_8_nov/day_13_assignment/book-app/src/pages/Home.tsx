...
function Home() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = bookStore.addChangeListener(() => {
      const storeBooks = bookStore.getBooks();
      setBooks(prevBooks => [...prevBooks, ...storeBooks]);
    });
  ...
  return (
    <Container className="py-4 fade-in">
      <RenderPropsComponent
        render={({ message }) => <h1 className="mb-4" style={{  padding: '10px', borderRadius: '5px' }}>{message}</h1>}
      />
